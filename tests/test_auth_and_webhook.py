...
async def test_google_connect_requires_oauth_config(client, settings):
    owner = await authenticate(client, settings, 111099, "oauthless")
...
async def test_group_connect_command_creates_workspace(client, app, settings, telegram_mock):
    await authenticate(client, settings, 111006, "connectadmin")
    ...
    payload = {
        ...
        "message": {
            "message_id": 12,
            "date": 1_700_000_051,
            "chat": {"id": -100205, "type": "group", "title": "Connect club"},
            "from": {
                "id": 111006,
                "is_bot": False,
                "first_name": "Connect",
    ...
    async with app.state.session_factory() as session:
        workspace = await session.scalar(select(Workspace).where(Workspace.name == "Connect club"))
